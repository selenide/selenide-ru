...
Действия и проверки (например `$("#name").click()` и `$$("#list").shouldHaveSize(1)`) теперь кидают ошибку, если браузер не открыт (или уже закрыт).
Т.е. сначала нужно открыть браузер (вызвать `open(url)`), а лишь потом работать с элементами 

Раньше подобные действия открывали браузер сами, если он не был уже открыт. И это поведение было иногда неожиданным. 
