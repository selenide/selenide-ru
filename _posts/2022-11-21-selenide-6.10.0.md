...
```java
File video = $("#skabEva").download(using(FOLDER)
  .withTimeout(Duration.ofSeconds(99))
  .withName("пыня мобшиза.MP4"));
```

Проблема в том, что незачем ждать так долго, если скачивание даже не началось (например, в момент клика ссылка была задизейблена).

Для ускорения падения теперь можно задать второй "increment timeout":
```java
File video = $("#skabEva").download(using(FOLDER)
...
При добавлении в URL эти символы не экранировались:
```
...
* browserup-proxy-core from 2.2.3 to 2.2.5, см. [ченджлог](https://github.com/valfirst/browserup-proxy/blob/master/CHANGELOG.md)
* nettyVersion from 4.1.82.Final to 4.1.85.Final
...
* JetBrains выпустила новую [IDE для автотестов](https://www.jetbrains.com/aqua/)!
* В Selenium появился свой [аналог WebDriverManager](https://www.selenium.dev/blog/2022/introducing-selenium-manager/). Кто-то уже попробовал?
