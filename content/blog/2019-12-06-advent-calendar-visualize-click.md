...
description: ""
category:
headerText: "Selenide Advent Calendar<br/>День 6"
tags: []
---
Привет!
...
> UPD Позже мы добавили методы, чтобы [кликать элементы с помощью JavaScript](/2020/09/26/selenide-5.15.0/#click-with-options):
> ```java
> $("#moving").click(usingJavaScript());
> ```
> 
> А также чтобы [кликать задизейбленные элементы](/2024/02/07/selenide-7.1.0/#force-click):
> ```java
> $("#disabled").click(usingDefaultMethod().force());
> ```
> 
> Они могут помочь кликнуть элемент, даже если он двигается, раздвигается или перекрывается другим элементом.
